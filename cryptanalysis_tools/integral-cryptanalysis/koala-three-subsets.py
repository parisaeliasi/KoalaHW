...
def main():
    ...
    K = []
    list_input = [int(i) for i in (sys.argv[2]).split(",")]
    L = [setMonomial(list_input,64)]

    #if an output coordinate is given then performe the search only for it
    if(len(sys.argv)==4):
        bit_number = int(sys.argv[3])
        time_start = time.time()
        res = bbdp(core_operation_injection_dict, core_operation_round_dict, K, L, bit_number)
        if(res==1):
            print(list_input)
            print("Bit " + str(bit_number) + ": " + str(res))
            time_end = time.time()
